...
btn.addEventListener('click', function(){
    if(email.value === '' || password.value === ''){
      ...
      erroSenha.innerHTML = `<p class="text-danger">Preencha todos os campos!</p>`;
    }
    else{
        onLoader();
        getAPI(url);
...
function verifica(admin){
    if(admin.email === email.value && admin.password === password.value){
        window.location.href = "/src/front/view/Home/home.html";
    }
    else{
        let erroSenha = document.getElementById("erroSenha");
        erroSenha.innerHTML = `<p class="text-danger">Usuario ou senha incorretos!</p>`;
        limpaPassword();
    }
}
